...
def parser(dialogue):
   ...
   # print("#############")
    ...
    dialogue  =  dialogue.replace( "'", "SINGLE_QUOTE")
    dialogue  =  dialogue.replace( "(", "BRACKET_LEFT ")
    dialogue  =  dialogue.replace( ")", " BRACKET_RIGHT") 
    dialogue  =  dialogue.replace( "[", "BRACKET_LEFT ")
    dialogue  =  dialogue.replace( "]", " BRACKET_RIGHT") 
    ...
    try:
      for token in tokenize.generate_tokens(buf.readline):  
        # if token.type == 54:
        if token.string in [":", "BRACKET_LEFT", "BRACKET_RIGHT"]   :
           if token.string == ":": 
                _lhs = p_tok.lower()
                #Robotic assistant:
           ...
           elif token.string == "BRACKET_LEFT": 
                skip = True

           elif token.string == "BRACKET_RIGHT": 
                skip = False
        ...
        if skip is False and token.string != "BRACKET_RIGHT":
            p_tok  = token.string
            p_type = token.type 
            p_tok = p_tok.replace( "SINGLE_QUOTE", "'")
            p_tok = p_tok.replace( "COMMA", ",")
            p_tok = p_tok.replace( "BRACKET_RIGHT", " ")
            p_tok = p_tok.replace( "BRACKET_LEFT", " ")
