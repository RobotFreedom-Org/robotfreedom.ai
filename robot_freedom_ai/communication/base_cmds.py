...
class BaseCmds(object):
    ...
    def __wait_for(self, message, i_timeout = 100000):
        """
    ...
    def clear_messages(self, message_type):
            """
            """

            done, val  = self.nerves.pop(message_type) 
            return [done, val]
    ...
    def do_direct_chat(self, arg):
        ...
        if self.nerves is None:
             self.nerves = Nerves('User')
        self.nerves.set("remote_cmd","chat:"+ arg) 

        i   = 0
        val = "error"

        while True:
            done, val  = self.nerves.pop("chat_responses_2") 
            if i == 1000 or done:
                break
            time.sleep(.2)
            i += 1
