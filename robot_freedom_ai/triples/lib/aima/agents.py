...
Environment ## An environment holds components, runs simulations
    XYEnvironment
        VacuumEnvironment
        WumpusEnvironment
...
class Object:
    ...
    You subclass Object to get the components you want.  Each object can have a
    .__name__  slot (used for output only)."""
    def __repr__(self):
        return '<%s>' % getattr(self, '__name__', self.__class__.__name__)

    def is_alive(self):
        """components that are 'alive' should return true."""
...
class Environment:
    ...
    The environment keeps a list of .components and .agents (which is a subset
    of .components). Each agent has a .performance slot, initialized to 0.
    Each object has a .location slot, even though some environments may not
    need this."""

    def __init__(self,):
        self.components = []; self.agents = []
    ...
    def add_object(self, object, location=None):
        ...
        track of components that are agents.  Shouldn't need to override this."""
...
class XYEnvironment(Environment):
    ...
    perceive components within a radius.  Each agent in the environment
    has a .location slot which should be a location such as (0, 1),
    and a .holding slot, which should be a list of components that are
    held """

    def __init__(self, width=10, height=10):
        update(self, components=[], agents=[], width=width, height=height)

    def components_at(self, location):
        "Return all components exactly at a given location."
        return [obj for obj in self.components if obj.location == location]

    def components_near(self, location, radius):
        "Return all components within radius of location."
        radius2 = radius * radius
        return [obj for obj in self.components
                if distance2(location, obj.location) <= radius2]

    def percept(self, agent):
        "By default, agent perceives components within radius r."
        return [self.object_percept(obj, agent)
                for obj in self.components_near(agent)]
    ...
    def execute_action(self, agent, action):
        if action == 'TurnRight':
            ...
            objs = [obj for obj in self.components_at(agent.location)
...
class EnvFrame(tk.Frame):
    def __init__(self, env, title='AIMA GUI', cellwidth=50, n=10):
        ...
        self.canvas.bind('<Button-2>', self.edit_components)
    ...
    def edit_components(self, event):
